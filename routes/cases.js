...
router.post('/', async (req, res) => {
  ...
  try {
    const { data: inserted, error } = await supabaseAdmin.from('chamados').insert([{
      user_id: userId,
      user_name: req.user?.name || req.user?.email || 'Desconhecido',
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    const { error } = await supabaseAdmin.from('chamados').update(fields)
      .eq('id', req.params.id);
    if (error) throw error;
...
router.get('/stats', async (req, res) => {
  try {
    // Must use supabaseAdmin — anon client has no auth session and is blocked by RLS
    const { data, error } = await supabaseAdmin
      .from('chamados')
      .select('id, user_id, nome, fabricante, categoria, status, data, hora, sn, integrador, cliente_final, adb_number, jira_key, created_at')
      .order('id', { ascending: false })
      .limit(1000);
    if (error) throw error;
    res.json(data || []);
